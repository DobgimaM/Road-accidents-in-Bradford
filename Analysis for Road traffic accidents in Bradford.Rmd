...
ggplot(data = casualties, #%>% mutate(accident_year = factor(accident_year))
       aes(x = accident_year), y = no_acc) +
  geom_bar(fill = '#44846fff') +
  facet_wrap(vars(casualty_severity)) +
  labs(title = 'Number of accident casualties by year',
       x = 'Accident year',
...
```
## 2. Casualties and rates by road user type
...
ggplot(data = temp %>% 
         filter(tag == 1),
       aes(x = casualty_type, fill = fct_infreq(accident_year))) +
  ...
  theme_classic() +
  theme(axis.text = element_text(size = 10, angle = 0, vjust = 0.5, hjust=0.5),
...
ggplot(data = temp %>% filter(tag ==2),
       aes(x = casualty_type, fill = fct_infreq(factor(accident_year)))) +
  geom_bar(position="dodge") +
  labs(title = 'Reported road fatalities by road user type in Bradford for 2018, 2019 and 2020 ',
         subtitle = '20-200 total casualties',
      x = 'Casualty type',
      y = 'Number of accidents') +
  theme_classic() +
...
ggplot(data = temp %>% filter(tag ==2),
       aes(x = casualty_type, fill = accident_year)) +
  geom_bar(position = 'dodge') +
  labs(title = 'Reported road fatalities by road user type in Bradford, 2020 compared with 2018',
         subtitle = 'less than 20 total casualties',
      x = 'Casualty type',
      y = 'Number of accidents') +
  theme_classic() +
...
df_type <- df1 %>% 
  select('2020','% change from 2018'= '2018', '% change from 2019'='2019') %>% 
  arrange(desc(`2020`))
...
ggplot(data = temp %>% 
         filter(tag == 1),
       aes(x = age_band_of_casualty, fill = sex_of_casualty)) +
  geom_bar(position="dodge") +
